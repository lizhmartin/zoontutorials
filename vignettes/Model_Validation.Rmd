...
```{r map, message=FALSE, warning=FALSE, fig.align='center', fig.height=7, fig.width=7}
...
We can also use our ecological knowledge - our internal model of the system - to question the model output. We expect the Carolina wren to be associated with X type of environment whcih we know is present in the southeast of the US and absent in the north and west. Our predicted distribution matches our expectations.

This is really the bare minimum we can do - check that our statistical model matches our internal model, or rather, current knowledge about the system. That isn't to say that we should be loyal to what we already know - part of buildin gmodels is to further scientific understanding. Indeed by exploring th epredicted distribution in the context of current understanding, new gaps in knowledge are often identified. These gaps can be further explored in future studies.


The next step in checking our model against current understanding is to, when possible, plot the predicted responses of our species of interest. With zoon, we can do this by using th eoutput module `ResponseCurve`. These curves show the predicted variation in probabilty of occurrence over the enviornmental gradients included in our model. When we include pure covariates, these relationships are informative and useful in the context of management. When we combine our covariates into principle componants with principle componant analysis, we lose the value that interpretation of these response curves provide.
...
The next step up in the model evaluation game is internal evaluation. Internal evaluation checks if our model is a good description of the data we fit it with. To do this in zoon, we use the process module Crossvalidate with k folds. To get the benfit of crossvalidation, we need to measure how good the fit is to the folded datasets left out. We can do this with the output module PerformanceMeasures.

```{r roc curve, message=FALSE, warning=FALSE, fig.align='center', fig.height=4, fig.width=4}
zoon_workflow <- workflow(occurrence = CarolinaWrenPA,
                          covariate = CarolinaWrenRasters,
                          process = NoProcess,
                          model = LogisticRegression,
...
```{r performance measure, message=FALSE, warning=FALSE, fig.align='center', fig.height=4, fig.width=4}
zoon_workflow <- workflow(occurrence = CarolinaWrenPA,
                          covariate = CarolinaWrenRasters,
...
Then, from the workflow object, we can get the performance measures by extracting the output from the workflow using hte accessor function `Output()`. This returns a list of all the performance measures

```{r performance measure extract, message=FALSE, warning=FALSE, fig.align='center', fig.height=4, fig.width=4}
...
The Crossvalidate process module randomly groups fold datasets (if presence-absence, the folds have equal proportions of presence and absences as in the dataset). However, we can more specifically ask how our model is at prediction. For example, we can do geographic cross-validation where we ask how our model can predict across geographic space. We do this by....
...
zoon_workflow <- workflow(occurrence = CarolinaWrenPA,
                          covariate = CarolinaWrenRasters,
                          process = Crossvalidate(k = 10),
                          model = list(LogisticRegression, mgcv),
                          output = PerformanceMeasures)
```
