...
```{r AUC, message=FALSE, warning=FALSE, fig.align='center', fig.height=4, fig.width=4}
...
# External validation
The gold standard of model evaluation is checking our predictions against an independent dataset. In zoon we do this by Chaining together our two datasets, one of which is a dataset we load into R from our local machine:

```{r external validation, eval=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.height=4, fig.width=4}
...
We can use the performance measuer discussed above to select between models. We can choose between modelling algorythms, like here we're comparing logistic regression and MaxNet:
